...
class Downloader:
    ...
    def Video_downloader(self, url, quality, file_path, i=""):
        """
        ...
        stream.download(
            output_path=file_path, filename_prefix="" if i == "" else f"{i} "
        )
        return 2

    def Playlist_downlaoder(self, url, quality, file_path):
        """
        ...
        if len(yt.video_urls) == 1:
            self.Video_downloader(yt.video_urls[0], quality, file_path, i=1)
            return
        ...
        for i in range(
            ...
            self.Video_downloader(yt.video_urls[i - 1], quality, file_path, i)

    def download_captions(self, url, file_path, lang_code):
        """
